    ...
    <div className="min-h-screen bg-neutral-50">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
            ...
            {activeTab === 'dashboard' && (
              <div className="flex gap-3">
                <Button
                  ...
                  className="flex items-center gap-2 bg-[#0066CC] hover:bg-[#0052A3] h-7 px-4 text-sm"
                >
        ...
        <TabsContent value="available">
          <Tabs value={availableSubTab} onValueChange={setAvailableSubTab} className="w-full">
            <TabsList className="h-10 bg-white mb-6">
              <TabsTrigger value="request" className="h-10">Request Session</TabsTrigger>
              <TabsTrigger value="slots" className="h-10">Available Slots</TabsTrigger>
              <TabsTrigger value="rsvp" className="h-10">Sessions to RSVP</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="rsvp">
              <div className="space-y-6">
                ...
                {filteredSessions.filter(s => s.rsvp_status === 'pending').length === 0 ? (
                  <div className="bg-white rounded-2xl border border-neutral-100 p-8 text-center">
                    <p className="text-neutral-400">No pending RSVPs</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {filteredSessions
                      .filter(s => s.rsvp_status === 'pending')
                      .map(session => {
                        const athlete = athletes.find(a => a.id === session.athlete_id);
                        const coach = coaches.find(c => c.id === session.coach_id);
